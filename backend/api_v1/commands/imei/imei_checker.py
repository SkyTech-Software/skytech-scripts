...
async def check_imei_by_number(imei: int, target_email: str):
    ...
    play_response = await play.send_request_to_play(imei, target_email)
    plus_response = plus.send_request_to_plus(imei, target_email)
    return ImeiResponse(orange=orange_response, tmobile=tmobile_response, play=play_response, plus=plus_response)